...
export const GrasslandApp: ThemeApp = props => {
  ...
  const main = props.contentErrorMessage ? <section role="alert"><h1>内容配置暂时无法读取</h1><p>{props.contentErrorMessage}</p></section> : props.route.kind === 'home' ? <HomePage memory={props.homeMemory} copy={{ eyebrow: '沿着地平线看回忆', title: '把辽阔的日子，慢慢收好。', subtitle: '阳光、远山与那些舍不得快进的片刻。' }} heroImage={assetUrl('media/themes/grassland/home-hero.webp')} viewerOpen={Boolean(props.session)} /> : props.route.kind === 'browse' ? <BrowsePage data={props.content} onOpen={props.onOpen} heroImage={assetUrl('media/themes/grassland/home-hero.webp')} /> : props.route.kind === 'albums' ? <AlbumsPage data={props.content} /> : album ? <AlbumPage album={album} albums={props.content.albums} onOpen={props.onOpen} /> : <section><h1>没有找到这个相册</h1><a href="#/">回到首页</a></section>;
