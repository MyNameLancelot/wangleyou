...
function MediaTile({ media, onOpen, album, variant = 'wide', eager }: { media: Media; album: Album; onOpen: OpenMedia; variant?: TileVariant; eager?: boolean }) {
  ...
  return <button type="button" className={[styles.mediaTile, variant === 'tall' ? styles.mediaTileTall : '', variant === 'masonry' ? styles.masonryTile : ''].filter(Boolean).join(' ')} onClick={() => onOpen(album, media.id)} aria-label={mediaLabel(album, media)}>
    <span className={styles.mediaThumb}>
      ...
      {isVideo(media) && <span className={styles.videoBadge}><Play aria-hidden="true" fill="currentColor" strokeWidth={0} />{durationText(media.duration) && <small>{durationText(media.duration)}</small>}</span>}
...
export function AlbumPage({ album, onOpen }: { album: Album; onOpen: OpenMedia }) {
  ...
  // 照片与视频排进同一条构建期顺序：视频沿用派生封面，并在网格里保留可辨识的播放标识。
  const tiles = album.media.map(media => {
    ...
    return {
      ...
      width: media.width,
      height: media.height,
      srcSet: source.srcSet?.map(item => ({ ...item, src: mediaUrl(item.src) })),
      alt: media.alt || media.description || '相册影像',
      label: mediaLabel(album, media),
  ...
  return <section className={styles.detail} aria-labelledby="album-title">
    ...
    {album.media.length ? <section className={styles.detailGallery} aria-label="相册影像" data-testid="album-media-grid">{tiles.length ? <MasonryPhotoAlbum photos={tiles} padding={8} onClick={({ photo }) => onOpen(album, photo.id)} componentsProps={{ container: { className: styles.masonryGrid } }} render={{ extras: (_props, { photo }) => photo.video ? <span className={styles.videoBadge}><Play aria-hidden="true" fill="currentColor" strokeWidth={0} />{durationText(photo.duration) && <small>{durationText(photo.duration)}</small>}</span> : undefined }} /> : <div className={styles.masonryGrid}>{album.media.map((media, i) => <MediaTile key={media.id} album={album} media={media} onOpen={onOpen} variant="masonry" eager={i < 3} />)}</div>}</section> : <div className={styles.empty}><span aria-hidden="true">☀</span><h2>这本相册正在整理</h2><p>新的影像会在这里出现。</p><a className={styles.primary} href="#/browse">返回留影</a></div>}
  </section>;
}
