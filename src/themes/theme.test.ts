...
import { THEMES, THEME_STORAGE_KEY, applyTheme, isThemeName, nextTheme, readTheme } from './index';
...
describe('主题运行时', () => {
  it('缺省与非法值都回退默认主题', () => {
    expect(readTheme(createStorage())).toBe('default');
    expect(readTheme(createStorage({ [THEME_STORAGE_KEY]: 'sunset' }))).toBe('default');
  ...
  it('非法主题名回退默认主题而不是写入脏值', () => {
    const root = createRoot();
    const storage = createStorage();
    expect(applyTheme('desert', root, storage)).toBe('default');
    expect(root.get('data-theme')).toBe('default');
    expect(storage.value(THEME_STORAGE_KEY)).toBe('default');
  });
  ...
  it('存储抛错时仍应用主题', () => {
    ...
    expect(readTheme(broken)).toBe('default');
  ...
  it('按顺序循环切换三套主题', () => {
    ...
    expect(nextTheme('unknown')).toBe('beach');
  });
});
