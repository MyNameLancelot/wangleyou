...
export const BeachApp: ThemeApp = props => {
  ...
  const main = props.contentErrorMessage ? <section role="alert"><h1>内容配置暂时无法读取</h1><p>{props.contentErrorMessage}</p></section> : props.route.kind === 'home' ? <HomePage memory={props.homeMemory} copy={{ eyebrow: 'LeYou • Growing Moments', title: '把有海风的日子，留在这里。', subtitle: '照片、视频与那些值得重看的片刻。' }} heroImage={mediaUrl('media/themes/beach/home-hero.webp')} memoryBackgroundImage={mediaUrl('media/themes/beach/home-memory.webp')} viewerOpen={Boolean(props.session)} /> : props.route.kind === 'browse' ? <BrowsePage data={props.content} onOpen={props.onOpen} heroImage={mediaUrl('media/themes/beach/home-hero.webp')} /> : props.route.kind === 'albums' ? <AlbumsPage data={props.content} /> : album ? <AlbumPage album={album} albums={props.content.albums} onOpen={props.onOpen} /> : <section><h1>没有找到这个相册</h1><a href="#/">回到首页</a></section>;
