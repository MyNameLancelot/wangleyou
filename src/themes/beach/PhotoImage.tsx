...
type Props = { src: string; alt: string; className?: string; width?: number; height?: number; eager?: boolean; draggable?: boolean };

/** 首次加载显示骨架，失败显示可读文案而不是空白；两态都不是仅靠颜色区分。 */
function ImageState({ src, alt, className, width, height, eager, draggable }: Props) {
  ...
  return <span className={`${styles.frame} ${className || ''}`} aria-busy={status === 'loading'}>
    ...
    {status === 'error'
      ...
      : <img src={src} alt={alt} width={width} height={height} loading={eager ? 'eager' : 'lazy'} decoding="async" draggable={draggable} onLoad={() => setStatus('ready')} onError={() => setStatus('error')} style={{ visibility: status === 'ready' ? 'visible' : 'hidden' }} />}
  </span>;
}
