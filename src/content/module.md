...
- 相册元信息的来源是每个照片目录的 `meta.json` 的 `album` 段：`title`、`date`、`description` 均可选，缺省分别取目录名后缀与目录名的年月（允许只写到月）。照片不写进配置：构建期脚本按 `topNN` 优先、其余文件名自然序生成顺序，并由文件名生成照片 ID。解析与错误定位在构建期脚本完成，本模块只消费生成结果。
