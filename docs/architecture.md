...
主题开关和音乐入口属于各自页面，不使用 `fixed` 或 `sticky`；可点击区域至少为 44px。查看器是库提供的模态层，不提供主题切换入口，背景为不使用媒体素材的纯黑不透明底；照片与视频在同一舞台内呈现，寄语由 captions 插件贴在画面底部居中。主题偏好和背景音乐偏好可保存到 `localStorage`，存储不可用时静默降级。
