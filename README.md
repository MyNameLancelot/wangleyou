...
已实现首页、影像浏览（年份分组与类型筛选）、相册索引与详情、照片大图与视频播放、连续播放、进度/音量/全屏、键盘与触屏操作、三套主题（默认 / 海边沙滩 / 旷野草原）切换与偏好记忆，以及内容校验和缩略图生成。

自动幻灯片、背景音乐和 App 套壳属于后续里程碑。设计基线与组件契约见 Penpot 文件，运行时主题差异由语义 Token 与装饰 Slot 表达。
