...
已实现首页、影像浏览（年份分组与类型筛选）、相册索引与详情、手动照片查看与视频播放、视频进度/静音/全屏、键盘与触屏操作、两套隔离主题（海边沙滩 / 旷野草原）切换与偏好记忆，以及内容校验和缩略图生成。查看器不连续播放，视频结束停留当前项。

查看器自动幻灯片和 App 套壳属于后续里程碑。首页背景音乐由海边、草原各自的主题私有资产提供。海边与草原分别拥有完整站点 UI、CSS、装饰和资产引用，只共享无 UI 的业务与交互契约；后续页面与交互设计以用户口述和活动规格为准。
...
- `meta.json` 的 `photos_meta.captions` 是可选的照片寄语：数组项只允许 `fileName`（目录内真实存在的图片文件名）与 `caption`，寄语为 1–60 个非空白字符，查看器会把有寄语的画面下方这条文案显示出来。文件名写错、重复登记同一文件、寄语为空或超长都会让构建失败并指出位置。
...
新增主题须建立独立目录，完整实现首页、悬浮主题开关、浏览、相册、查看器、CSS、装饰和资产引用；不得导入其他主题或共享 React UI。站点不提供常驻全局顶部导航，内容入口和返回链接由各主题页面独立维护。主题只共享 content、playback、albums、routing 的无 UI 类型与纯交互契约。视觉特效优先评估轻量既有库，并记录体积、依赖、许可与兼容回退。当前治理决策见 [ADR 0003](docs/decisions/0003-retire-penpot-design-governance.md)。
