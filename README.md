...
已实现首页、影像浏览（年份分组与类型筛选）、相册索引与详情、照片大图与视频播放、连续播放、进度/音量/全屏、键盘与触屏操作、两套隔离主题（海边沙滩 / 旷野草原）切换与偏好记忆，以及内容校验和缩略图生成。

自动幻灯片、背景音乐和 App 套壳属于后续里程碑。海边与草原分别拥有完整站点 UI、CSS、装饰和资产引用，只共享无 UI 的业务与交互契约；后续页面与交互设计以用户口述和活动规格为准。
